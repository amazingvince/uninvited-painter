...
export type ClientMsg =
  ...
  | { t: "deal" } // QM only
  | { t: "seen" }
  | { t: "live"; points: StrokePoints } // in-progress stroke batch (ephemeral)
  | { t: "liveClear" } // undo within the grace window
  | { t: "commit"; points: StrokePoints }
...
export type ServerMsg =
  ...
  | { t: "live"; playerId: string; colorIndex: number; points: StrokePoints }
...
export interface PublicRoomState
  extends Omit<RoomState, "round" | "usedWords" | "fakeCounts" | "qmIndex"> {
...
export function redactState(state: RoomState, viewerId: string): { state: PublicRoomState; you: YouView } {
  ...
  if (round) {
    ...
    const revealed = state.phase === "reveal" || state.phase === "closed";
  ...
  const { round: _r, usedWords: _u, fakeCounts: _f, qmIndex: _q, ...rest } = state;
  ...
  return {
    state: { ...rest, archive, round: publicRound },
    you: {
      playerId: viewerId,
      role,
      word,
