...
describe("private AI job store", () => {
  it("stores source, status, rendition, and promoted archive artwork", async () => {
    ...
    expect(await promoteRendition(env, JOB_ID, ARCHIVE_ID, 2)).toBe(true);
    expect(bucket.objects.get(archiveRenditionKey(ARCHIVE_ID, 2))).toBeDefined();
  });
  ...
  it("writes exact content types and cache metadata", async () => {
    ...
    await promoteRendition(env, JOB_ID, ARCHIVE_ID, 2);
    ...
    expect(
      bucket.objects.get(archiveRenditionKey(ARCHIVE_ID, 2))?.options
        ?.httpMetadata,
    ).toEqual({
      contentType: "image/jpeg",
  ...
  it("rejects invalid identifiers and status shapes", async () => {
    ...
    await expect(
      promoteRendition(env, JOB_ID, "../escape", 2),
    ).rejects.toThrow(/archive/i);
    await expect(
      promoteRendition(env, JOB_ID, ARCHIVE_ID, 0),
    ).rejects.toThrow(/round/i);
  });

  it("returns null for missing or corrupted objects and false for missing promotion", async () => {
    const { env, bucket } = setup();
    ...
    expect(await promoteRendition(env, JOB_ID, ARCHIVE_ID, 2)).toBe(false);
